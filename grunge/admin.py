...
class PlayListAdmin(admin.ModelAdmin):
    list_display = ("name", "track_admin_link")
    list_filter = ("name",)
    search_fields = ("uuid", "name")
    fields = ("name", "track_admin_link")
    readonly_fields = ("track_admin_link",)
    ordering = ("name",)

    def get_queryset(self, request):
        self.request = request
        queryset = super().get_queryset(request)
        return queryset.annotate(track_count=Count("track"))
    
    def track_admin_link(self, playlist):
        return get_admin_url(
            PlayList,
    ...
    track_admin_link.short_description = _("Tracks")
    track_admin_link.admin_order_field = "track_count"